...
THIS IS A STEADY DQN TRAINING SCRIPT FOR ATARI BREAKOUT GAME

AUTHOR: JEFFLIULAB, CLAUDE 3.7, CHATGPT 4o-mini-high 

这个脚本和4_dqn_2M.py没有任何区别, 是一个稳定训练版本。
这个脚本的默认训练次数是200万次, 默认的最终EPSILON是0.01。
这个脚本推荐使用参数导入和断点恢复开始训练，不推荐从头开始。

使用方法：
先确定好checkpoint位置和FINAL EPSILON的数值, 然后确定Target Iteration Times后, 执行参数运行脚本:
python src/5_dqn_Continue.py --resume [Checkpoint File Path] --iterations [Target Iteration Times]

对于RL课程的Final Report, 只交这一个脚本就够了。

已经训练的内容:(都在output中,第一阶段为4_dqn_2M的结果, 单独存储在docs/output4中)
第一阶段: 前100万次, 最低epsilon0.05, 线性衰减; 100-200万次, 最低epsilon0.05 (已完成，保存为)
第二阶段: 从checkpoint 1950000开始, 调整epsilon为0.03, 继续训练到300万次
"""

EPSILON_FINAL_SETTING = 0.01
...
# 然后sample一个batch做一次训练
    ...
    try:
        ...
        VIDEO_FOLDER_TIMESTAMP_PATH = f"output/videos/run_{timestamp}"  # 或者 f"output/videos/iter_{num_iterations}"


        video_env = gym.wrappers.RecordVideo(
            initialize_environment(render_mode="rgb_array"),
            video_folder=VIDEO_FOLDER_TIMESTAMP_PATH,
